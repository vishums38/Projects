...
setcookie("email", "$email", time()+3600);
 ...
 // Account details
	...
	$message = "THANK YOU FOR REGISTERING WITH US $username  GREEN-IT MANAGEMENT ! OTP to Register OTP : $otp ";
    ...
    define("DB_DATABASE", "id7339349_greenit");
      $db = mysqli_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_DATABASE);
             $db2=mysqli_select_db($db,DB_DATABASE);
                $res=mysqli_query($db,"INSERT INTO `login` (`username`, `password`, `com_name`, `email`, `mob_no`) VALUES ('$username', '$password', 'KUMARESHWARA_COM','$email','$num');");
                       if($res)
                        {
...
<div class="testbox">
  ...
  <label id="icon" for="name"><i class="icon-shield"></i></label>
  <input type="text" name="mobile" id="name" placeholder="Mobile-Number" required/>
  
  
   <p>By clicking Register, you agree on our <a href="#">terms and condition</a>.</p></br>
    <button class="btn waves-effect waves-light puls" type="submit"  name="send" value="sendOTP" class="registerbtn">SENDOTP</button></br>
  </form>
  <form action="" method="POST">
		<h1>Enter OTP</h1>
		 <label id="icon" for="name"><i class="icon-shield"></i></label>
  <input type="text" name="otp" id="name" placeholder="OTP" required/>
  	<button class="btn waves-effect waves-light puls" type="submit" value="REGISTER" name="verify">REGISTER</button>
  	</br>
